...
export function renderGenerateAllVsixMjsSource(product: ScriptsProduct): string {
    const meta = productScriptsMeta(product);
    const lines = [
        GENERATED_SCRIPTS_BANNER.trimEnd(),
        '',
        `/**`,
        ` * VSIX generate-all: top-level *${meta.dslExtension} only; uses extension embed CLI.`,
        ` * Usage: node ./generated/${meta.productName}/scripts/generate-all-vsix.mjs`,
        ` */`,
