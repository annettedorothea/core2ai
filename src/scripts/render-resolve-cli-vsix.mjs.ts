...
/** VSIX-only CLI resolve: extension folder scan. Never reads env or monorepo paths. */
export function renderResolveCliVsixMjsSource(product: ScriptsProduct): string {
    const meta = productScriptsMeta(product);
    const lines = [
        GENERATED_SCRIPTS_BANNER.trimEnd(),
        '',
        `/**`,
        ` * Resolve ${meta.productName} CLI from the newest installed VS Code / Cursor extension embed.`,
